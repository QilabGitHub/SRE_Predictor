...
~/miniconda3/envs/py2/bin/python generate_regions.py $gene_list $Enhancer_region $Output_Path
echo "Done!!"
...
do
	java -jar /home/users/xqlin/Tools_and_Files/juicer/scripts/juicer_tools.jar dump observed VC_SQRT $HiChIP_hic $chrom_num $chrom_num BP $HiChIP_res $Output_Path/"HiChIP_VC_SQRT"/"GM12878_chr"$chrom_num"_matrix_"$HiChIP_res"bp_Cohesin_VC_SQRT_norm.txt"
	java -jar /home/users/xqlin/Tools_and_Files/juicer/scripts/juicer_tools.jar dump expected VC_SQRT $HiChIP_hic $chrom_num BP $HiChIP_res $Output_Path/"HiChIP_VC_SQRT"/"GM12878_chr"$chrom_num"_expected_"$HiChIP_res"bp_Cohesin_VC_SQRT_norm.txt"
	###need python3
	~/miniconda3/envs/py3/bin/python retrieve_hic_pets.py  $HiChIP_hic $chrom_num $HiChIP_res VC_SQRT $Output_Path/"HiChIP_VC_SQRT"/"GM12878_chr"$chrom_num"_"$HiChIP_res"bp_Cohesin_VC_SQRT.bedpe" 
done	
echo "Done!!"
...
~/miniconda3/envs/py2/bin/python bed2bedpe.py $Output_Path/$gene"/Promoter_enhancers.bed" $Output_Path/$gene"/Promoter_enhancers.bedpe" 4 -10
~/bin/bedtools pairtopair -type both -a $Output_Path/$gene"/Promoter_enhancers.bedpe" -b $Output_Path/"HiChIP_VC_SQRT"/"GM12878_"$chrom"_"$HiChIP_res"bp_Cohesin_VC_SQRT.bedpe" -is > $Output_Path/$gene"/Promoter_enhancers_HiChIP_"$HiChIP_res"bp_VC_SQRT.bedpe"
...
~/miniconda3/envs/py2/bin/python retrieve_TF_HMs_DoubleElement_matrix.py $Output_Path/$gene"/Enhancers.bed" $Output_Path/$gene"/Enhancers_selected_TFs_HMs_Zscore" $HM_TF_BigWig_Folder 0 1
echo "Done!!"
